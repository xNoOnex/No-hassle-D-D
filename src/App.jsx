...
export default function App() {
  ...
  const handleNetworkData = (data) => {
    if (role === 'player') {
      // Player receives full state overwrite from DM
      setGameState(data);
    } else if (role === 'dm') {
      // DM receives action requests from Players (e.g. dice rolls)
  ...
  const dmUpdateQuest = () => {
    if (!network) return;
    const newState = { ...gameState, quest: `Slay the Goblins (Turn ${gameState.turn + 1})`, turn: gameState.turn + 1 };
  ...
  return (
    <div className="container">
      <div className="card">
        ...
        <div className="flex-center" style={{color: status.includes('Active') || status.includes('Connected') ? 'var(--success)' : 'var(--text-muted)', marginBottom: '20px'}}>
        ...
        {role === 'dm' ? (
          <button className="btn-primary" onClick={dmUpdateQuest}>Broadcast New Objective</button>
        ) : (
